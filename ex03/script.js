...
function renderizarTabela() {
  ...
  InfoTabela.forEach(usuario => {
    ...
    linha.innerHTML = `
      ...
      <td>
        <button onclick="editarDados(${usuario.id})">
          ✏️ Editar
        </button>

        <button onclick="excluirDados(${usuario.id})">
          🗑️ Excluir
        </button>
      </td>
    `;
  ...
  linha.innerHTML = `
    ...
    <td>
      <select id="edit-sexo-${id}">
        ...
        <option value="feminino"
          ${usuario.sexo === "feminino" ? "selected" : ""}>
          Feminino
        </option>

        <option value="outro"
          ${usuario.sexo === "outro" ? "selected" : ""}>
          Outro
        </option>
      </select>
    </td>
    ...
    <td>
      <select id="edit-turma-${id}">
        <option value="turma_a"
          ${usuario.turma === "turma_a" ? "selected" : ""}>
          1ESAN (A)
        </option>

        <option value="turma_b"
          ${usuario.turma === "turma_b" ? "selected" : ""}>
          1ESBN (B)
        </option>

        <option value="turma_c"
          ${usuario.turma === "turma_c" ? "selected" : ""}>
          1ESCN (C)
        </option>
      </select>
    </td>
    ...
    <td>
      ...
      <button onclick="renderizarTabela()">
        ✖ Cancelar
      </button>
    </td>
  `;
...
function salvarEdicao(id) {
  ...
  const novoNome =
    document.getElementById("edit-nome-" + id).value.trim();

  const novoEmail =
    document.getElementById("edit-email-" + id).value.trim();

  const novoSexo =
    document.getElementById("edit-sexo-" + id).value;
